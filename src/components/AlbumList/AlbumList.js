...
function AlbumList(props) {
    ...
    //fetching albums while app mounts
    useEffect(() => {
        //loading true to show spinner
        setLoading(true);
        setTimeout(() => {
            getData();
            //loading false to stop the spinner and render the albums
            setLoading(false)
        }, 1000)
        ...
        <>
            ...
            {toggleComponent ? <ImageList next={next} prev={prev} index={index} setIndex={setIndex} carousel={carousel} showCarousel={showCarousel} carouselToggler={carouselToggler} setCarouselToggler={setCarouselToggler} searchToggler={searchToggler} setSearchToggler={setSearchToggler} search={search} showSearch={showSearch} update={update} setUpdate={setUpdate} setAddImageToggler={setAddImageToggler} searchResults={searchResults} setSearchResults={setSearchResults} deleteImg={deleteImg} updateImg={updateImg} loading={loading} getImages={getImages} setLoading={setLoading} images={images} addImageToDB={addImageToDB} album={album} outsideAlbum={outsideAlbum} addImageToggler={addImageToggler} addImgHandler={addImgHandler} /> :
                <section className="w-full h-fit flex items-center justify-center flex-col">
                    ...
                    {/* conditionally rendering AlbumForm */}
                    ...
                    {/* if loading then showing spinner */}
